...
export function OrchestratorDiagram({
  ...
  const w = 640;
  const h = 320;
  const docX = 46, docY = h / 2;
  const routerX = 210, routerY = h / 2;
  const nodeX = 430;
  const resultX = 592, resultY = h / 2;
  const n = models.length;
  const gap = (h - 60) / (n - 1);
  const nodeYs = models.map((_, i) => 30 + i * gap);

  return (
    <svg viewBox={`0 0 ${w} ${h}`} className={["tp-orchestrator", className].filter(Boolean).join(" ")} role="img" aria-label="Схема AI-оркестратора">
